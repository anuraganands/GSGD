...
%     curFoldConsistentIdx = consistentIdx;
    ...
    EinGSGDtail = GradientDescentTail;
    EvSGDtail = [];
    EvGSGDtail = [];
...
fprintf('Final Eout for GSGD\n');
[SR_GSGD, NFC] = PrintFinalResults([],PocketGoodWeights,inputVal,givenOut, false, activation);
...
%     dlmwrite(outFileMicroEin,PlotEoutSRsgd,'-append');
    ...
    %Save macro Ein
    outFileGSGD = ['Result/' cell2mat(iAlgo) '/' file_used 'GSGD' '.csv' ];
    GSGDplot = [runs GSGDplot SR_GSGD NFC]; %1+L+2 %column size
    if(runs == 1)
        dlmwrite(outFileGSGD,GSGDplot);  
    else
        dlmwrite(outFileGSGD,GSGDplot,'-append');
    end
...
[SRsGSGD, fileData1, microEin] = aggregateAnalysis(outFileGSGD,AggregateAnalysisL, outFileMicroEin, T, totalRun*4);
[SRsSGD, fileData2, ~] = aggregateAnalysis(outFileSGD,AggregateAnalysisLsgd, outFileMicroEin, T, totalRun*4);
%microEinGSGD :cell(4,2) %     
...
dlmwrite(outFileGSGD,[-1 SD],'-append');
dlmwrite(outFileSGD,[-1 SD],'-append');


%compare digested/analysed files)
[GSGDwinsPC, SGDwinsPC, GSGDwin, SGDwins] = compareAlgorithms(SRsGSGD, SRsSGD);
fprintf('Win %% for GSGD: %0.1f (%d/%d)\n',GSGDwinsPC, GSGDwin, totalRun);
...
aggregateValues = [-1 fill GSGDwinsPC]; 
dlmwrite(outFileGSGD,aggregateValues,'-append');
...
% % 
% % biGSGD = EinGSGDtail.getBestIdx;
% % fprintf('Min Ein for GSGD: %0.4f\n', EinGSGDtail.Ebests(biGSGD));
% % fprintf('Min Eval for GSGD: %0.4f\n', min(EvGSGDtail));
...
%%
function [GSGDwinsPC, SGDwinsPC, GSGDwins, SGDwins] = compareAlgorithms(SRsGSGD, SRsSGD)
    GSGDwins = 0;
    SGDwins = 0;
    N = min(size(SRsGSGD,1),size(SRsSGD,1));
    for i = 1:N
        if (SRsGSGD(i)>SRsSGD(i))
            GSGDwins = GSGDwins+1;
        end
        
        if (SRsSGD(i)>SRsGSGD(i))
            SGDwins = SGDwins+1;
        end
    end
    GSGDwinsPC = GSGDwins*100/N;
